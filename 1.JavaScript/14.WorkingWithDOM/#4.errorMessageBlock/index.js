...
taskBlock.addEventListener('submit', event => {
    ...
    const errorMessage = message => {
        const messageBlock = document.querySelector('.error-message-block')
        messageBlock ? messageBlock.remove() : false
    ...
    } else if (tasks.some(item => item.text === inputValue)) {
        errorMessage('A task with the same name already exists')
    } else {
        const task = {
            id: `${Date.now()}`,
            completed: false,
